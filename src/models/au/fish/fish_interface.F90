...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    subroutine fish_rate_dist_initialize(self,configunit)
        ...
        !call self%register_diagnostic_variable(self%id_BOT_FEED,'bottom_activity','g m-2','bottom activity', & ! not sure about act_as_state_variable
        !                  act_as_state_variable=.true.,output=output_instantaneous,domain=domain_bottom,source=source_do_column,missing_value=0.0_rk)
    ...
    subroutine fish_rate_dist_do(self,_ARGUMENTS_DO_)
        ...
        _LOOP_BEGIN_
           ...
           do i_prey=1,self%nprey
                ...
                !write (*,*) self%id_targets_preyP(i_prey)%state_index, self%id_targets_preyP(i_prey)%sms_index
                !stop 'kurt'

                !write (*,*) i_z, i_prey, lossDW(i_z,i_prey)
           end do
