...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    type,extends(type_base_model),public :: type_fish_cohort
       ...
       type (type_horizontal_diagnostic_id), allocatable     ::  id_g_mass(:)
       type (type_horizontal_diagnostic_id), allocatable     ::  id_dist(:,:)
       
       type (type_horizontal_diagnostic_id), allocatable     ::  id_prey_loss_DW(:)
       type (type_horizontal_diagnostic_id), allocatable     ::  id_prey_loss_P(:)
       type (type_horizontal_diagnostic_id), allocatable     ::  id_prey_loss_N(:)
       type (type_horizontal_diagnostic_id), allocatable     ::  id_pas_rates(:,:)
...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    allocate(self%id_Z(self%cht_nlev))
    allocate(self%id_dz(self%cht_nlev))
    allocate(self%id_uTm(self%cht_nlev))
    allocate(self%id_pas_rates(self%cht_nlev, self%cht_npas)
    ...
    allocate(self%id_g_mass(self%cht_nC+self%cht_nc_init))
    allocate(self%id_dist(self%cht_nC+self%cht_nc_init,self%cht_nlev))
    
    do i_nc=1,self%cht_nC+self%cht_nc_init
       write (index_nc,'(i0)'), i_nc
       call self%register_state_variable(self%id_N(i_nc),'N'//trim(index), '[m^-2]','abundance of cohort number '//trim(index_nc),initial_value= 0._rk, minimum=0.0_rk)
       ...
       call self%register_diagnostic_variable(self%id_g_mass(i_nc),'reproductive_effort'//trim(index),'gDW','reproductive effort of cohort number '//trim(index_nc),act_as_state_variable=.true.,domain=domain_surface,output=output_none)
       do i_z=1,self%cht_nlev
           write(index_z,'(i0)') i_z
           call self%register_diagnostic_variable(self%id_dist(i_nc,i_z),'weight_cohort_'//trim(index_nc)//'_at_z'//trim(index_z),'-','proportion time spent at depth '//trim(index_z)//' by cohort '//trim(index_nc),act_as_state_variable=.true.,domain=domain_surface)
       end do
    end do
...
!xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    ...
    real(rk), dimension(self%cht_nlev, &
        self%cht_nC+self%cht_nc_init)                   :: dist
    ! derivatives --------------------------------------------------------------------------------------------------
    ...
    real(rk), dimension(self%cht_nC+self%cht_nc_init)   :: mu                  
    ...
    real(rk), dimension(self%cht_nC+self%cht_nc_init, &
        self%cht_nlev)                                  :: I_r,         eta_z,      eta,        E_m,        mu_c_M
    real(rk), dimension(self%cht_nC+self%cht_nc_init, &
        self%cht_nC+self%cht_nc_init, self%cht_nlev)    :: A_c_M,       eta_c
    ...
    real(rk), dimension(self%cht_nC+self%cht_nc_init, &
        self%cht_nC+self%cht_nc_init, self%cht_nlev)    :: I_r_pis,     I_r_N_pis,  N_ass_pis,  I_r_P_pis,  N_ass_pis
    ...
    !===============================================================================================================
        ...
        _GET_HORIZONTAL_(self%id_g_mass,g_mass)                                                                     ! get cohort reproductive effort (gonads)
        !===========================================================================================================
        ...
        _GET_HORIZONTAL_(self%id_dist,dist)                                                                         ! get current fidh distributions
    ...
    !===============================================================================================================
        ...
        eta_c = A_c_M*spread(dist*spread((r_mass+i_mass)*N, DIM=2, NCOPIES=self%cht_nlev), DIM=2, NCOPIES=nC_fin)   ! depth-specific encounter rates with fish prey (dimensions: nC_fin x nC_fin x nlev)
        ...
        E_m = E_m_P*pi                                                                                              ! depth-specific realized metabolism (dimensions: nC_fin x nlev)
        
        mu_c_M = sum(A_c_M*spread(dist*spread(N, DIM=2, NCOPIES=self%cht_nlev)/(1.0_rk+H_M*eta) &
            , DIM=1, NCOPIES=nC_fin), 2)*p_i                                                                        ! depth-specific mortality from piscivory (dimensions: nC_fin x nlev)
        ...
        endwhere
        E_m_corr = sum(E_m, 2)*max(self%cht_PD_Ref/PD,self%cht_ND_Ref/ND)                                           ! correct respiration to maintain stochiometric equilibrium (dimensions: nC_fin)
        !===========================================================================================================
        ...
        _SET_HORIZONTAL_DIAGNOSTIC_(self%id_g_mass,g_mass)                                                          ! update reproductive effort diagnostic
        !===========================================================================================================
        ...
        _SET_HORIZONTAL_DIAGNOSTIC_(self%id_dist,p_i)                                                               ! update fish distributions
    !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    !===============================================================================================================
        ...
        !:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
            ! 4.2.1 phosphorous
            !-------------------------------------------------------------------------------------------------------
            P_exc = spread(PD/self%cht_PD_Ref, DIM=2, NCOPIES=self%cht_nlev) * E_m                                  ! calculate rates of P excretion (dimensions: nC_fin x nlev)
            !:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
            ! 4.2.2 nitrogen
            !-------------------------------------------------------------------------------------------------------
            N_exc = spread(ND/self%cht_ND_Ref, DIM=2, NCOPIES=self%cht_nlev) * E_m                                  ! calculate rates of N excretion (dimensions: nC_fin x nlev)
        !===========================================================================================================
        ! 4.3 egestion
        !:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
            ! 4.3.1 dry weight
