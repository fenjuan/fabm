...
!  food_integral module: added food source together, which will be used in distribution module
   type,extends(type_base_model),public :: type_depth_integral
      type (type_horizontal_diagnostic_variable_id) :: id_totFood
      type (type_state_variable_id)                 :: id_food
...
!      real(rk) :: z_top, z_bot, w_top, w_bot    ! this will be automatically calculated depending on real case
   ...
   contains
    ...
    subroutine depth_integral_initialize(self,configunit)
      ...
      call self%register_state_dependency(self%id_food, 'food', 'g/m-3', 'variable to depth-integrate')
!      call self%register_dependency(self%id_weights,'weights','-','weights for vertical integration')
      call self%register_diagnostic_variable(self%id_totFood,'total_food','g/m2','total_food',missing_value=0.0_rk, &
...
! fen: distribute feedback to local trophic levels, such as zooplankton, nutrients, detritus, etc.
   subroutine depth_integral_rate_distributor_initialize(self,configunit)
      ...
      !     couple to the food
!      call self%request_coupling(self%id_food,'fish_food')
...
!!!!! fen: calculate the sum of the vertial layer
!!!   
   subroutine depth_integral_do(self,_ARGUMENTS_VERTICAL_)
      ...
      _VERTICAL_LOOP_BEGIN_
        ...
        case(0)
             ...
             tot_food = tot_food + food * thickness  !! for now tot_food is g/m2
        case(1)
            if (temp < self%min_temp .OR. temp > self%max_temp) then
               tot_food = tot_food + 0.0_rk   ! if out of habitat, not predating on current place
            else 
               _GET_(self%id_food, food)
             tot_food = tot_food + food * thickness  !! for now tot_food is g/m2
            end if
        ...
        case(3)
            if ( temp > self%min_temp .AND. temp < self%max_temp &
                ...
                _GET_(self%id_food, food)
                tot_food = tot_food + food * thickness
            else
                tot_food = tot_food + 0.0_rk
            end if
        end select         
      ...
      _SET_HORIZONTAL_DIAGNOSTIC_(self%id_totFood,tot_food)
   ...
   subroutine vertical_distribution_do(self,_ARGUMENTS_DO_)
      ...
      real(rk) :: weight, food
      real(rk) :: tot_food = 1.0_rk  ! this should be a dependency on intergral, which will get the total amount of food in whole water column
