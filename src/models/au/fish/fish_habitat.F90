...
module fish_habitat
   ...
   type,extends(type_base_model),public :: type_vertical_distribution
      ...
      type (type_horiziontal_dependency_id)   :: id_predator    !  couple to the non-local variable, will be used to calculated rates
   ...
   type,extends(type_base_model),public :: type_depth_integral
      type (type_horizontal_diagnostic_variable_id) :: id_integral, id_tmass, id_mmass   ! tmass is the total mass concentration of prey, and mmass is the mean concentration of prey
...
contains
   ...
   subroutine depth_integral_initialize(self,configunit)
      ...
      call self%register_diagnostic_variable(self%id_tmass,'food','g/m2','food',missing_value=0.0_rk, &
         act_as_state_variable=.true.,domain=domain_bottom,source=source_do_column)
   ...
   subroutine depth_integral_do(self,_ARGUMENTS_VERTICAL_)
      ...
      real(rk) :: local,weight,thickness,integral,tot_mass
      ...
      _VERTICAL_LOOP_BEGIN_
         ...
         tot_mass = tot_mass + local * thickness
      _VERTICAL_LOOP_END_
      _SET_HORIZONTAL_DIAGNOSTIC_(self%id_integral,integral)
      _SET_HORIZONTAL_DIAGNOSTIC_(self%id_tmass,tot_mass)
      print*, 'food', tot_mass
